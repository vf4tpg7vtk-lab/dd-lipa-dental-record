...
function scheduleAutoSave() {
  ...
  state.autoTimer=setTimeout(()=>{
    ...
    state.autoSaved=true; render();
    setTimeout(()=>{ state.autoSaved=false; render(); },2200);
  },1500);
}
...
function updateTreatment(i, patch) {
  ...
  updatePatient({treatments});
}

function addTreatment() {
  updatePatient({treatments:[...(state.patient.treatments||[]),newTreatment()]});
}

function removeTreatment(i) {
  updatePatient({treatments:state.patient.treatments.filter((_,idx)=>idx!==i)});
}
...
function saveLocal() {
  ...
  LS.save(state.patient); state.localList=LS.list(); toast('success','Saved to browser storage ✓');
}

function downloadFile(p=state.patient) {
...
function renderTreatmentTable() {
  const wrap = h('div',{class:'treatment-wrap'});
  ...
  const creditTh = h('th'); creditTh.colSpan=3; creditTh.textContent='CREDIT'; tr1.appendChild(creditTh);
  ...
  treatments.forEach((row,i)=>{
    const tr=h('tr');
    const fields=[['date','100px','date'],['toothNo','50px','text'],['description','','text'],['time','70px','text'],['debit','90px','text'],['creditDate','90px','date'],['creditAmount','90px','text'],['balance','90px','text']];
    fields.forEach(([key,w,type])=>{
      const td=h('td');
      const inp=h('input',{type,class:'t-input',value:row[key]||'',onchange:e=>updateTreatment(i,{[key]:e.target.value})});
      if(w)td.style.width=w;
      td.appendChild(inp); tr.appendChild(td);
    });
    const delTd=h('td');
    delTd.style.cssText='text-align:center;padding:4px';
    const delBtn=h('button',{class:'btn btn-icon',onclick:()=>removeTreatment(i),style:{color:'#e74c3c',background:'none',border:'none',cursor:'pointer',fontSize:'18px'}});
  ...
  addBtn.textContent='+ Add Treatment Row';
...
function renderPatientForm() {
  ...
  fields.forEach(f=>{
    ...
    inp.addEventListener('input',e=>updatePatient({[f.key]:e.target.value}));
...
function renderHome() {
  ...
  [
    {icon:'🖥️',title:'Browser Storage',count:`${LS.count()} record${LS.count()!==1?'s':''} on this device`,action:()=>{ state.listSource='local'; setView('list'); }},
    {icon:'📁',title:'Local File',count:'Download / import .json files',action:()=>document.getElementById('upload-input').click()},
  ...
  sb.appendChild(sg); storageCard.appendChild(sb); frag.appendChild(storageCard);
...
function renderRecord() {
  ...
  const dlBtn=h('button',{class:'btn btn-ghost btn-sm',onclick:()=>downloadFile()});
  ...
  // Status bar
  const status=h('div',{class:'record-status-bar'});
  ...
  const idSpan=h('span',{style:{marginLeft:'auto',fontSize:'10px',color:'var(--muted)'}}); idSpan.textContent=`ID ${p.id}`;
  status.appendChild(idSpan);
  ...
  txb.appendChild(renderTreatmentTable());
  txCard.appendChild(txh); txCard.appendChild(txb); frag.appendChild(txCard);
  ...
  const b2=h('button',{class:'btn btn-ghost',onclick:()=>downloadFile()}); b2.textContent='⬇ Download .JSON';
